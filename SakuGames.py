...
def login_user():
    ...
    if os.path.exists(USER_DATA_FILE):
        ...
        if username in users and check_password(users[username], password):
            ...
            subprocess.Popen(['python', 'Main.py'])  # Open Main.py in a new process
        else:
...
window_start.iconbitmap(ICON_FILE_START)  # Set icon
...
window_register.iconbitmap(ICON_FILE_REGISTER)  # Set icon
window_register.withdraw()
...
window_login.iconbitmap(ICON_FILE_LOGIN)  # Set icon
